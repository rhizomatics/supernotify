...
Home Assistant default camera entities have built-in device tracking, so the entity state will be `idle`, `recording` or `unavailable` if the camera is not available. Supernotify can use this state to switch to an alternative camera if the first choice isn't up. It's also possible to associate a separate `device_tracker` to the camera, for example using a Unifi or similar integration to track that the camera network device is up.
...
- Configuration documentation for [Camera Schema](https://supernotify.rhizomatics.org.uk/latest/developer/reference/schemas/Camera_Definition/index.md).
...
The [Media Source](https://www.home-assistant.io/integrations/media_source/) integration can be configured to include the Supernotify media directory, so snapshots used for e-mail, mobile push etc can be viewed via the Home Assistant UI. This integration is switched on as part of the Home Assistant default configuration so all you need is to declare the `media_dirs`, like this:
...
If you don't want to have images reprocessed, perhaps for performance or other reasons, then use this configuration in the delivery or transport configuration, or in the action call.
...
`reprocess` defaults to `always`. It can also be set to `preserve` where the original image with any comments and other metadata is preserved, and then `jpeg_opts` or `png_opts` applied on top.
