...
- The [Apple push fix](https://supernotify.rhizomatics.org.uk/latest/recipes/fix_frigate_apple_push/index.md) is built in, with an option to turn it off
...
- **Package module inside Supernotify**, with `mqtt` and `frigate` as `after_dependencies`, only offered when both are loaded. A thin *Frigate SuperNotifier* HACS repository can follow later for visibility
- **Config subentry per notification profile** of the Supernotify entry - a Frigate instance and a set of cameras with their own filters and notification. Most users have one; someone wanting the driveway treated differently from the garden adds a second
- **Frigate settings discovered** from the existing `frigate` config entry. The MQTT topic prefix, proxy client ID and base URL are read from the Frigate entry and Home Assistant's external URL, replacing the blueprint's `base_url`, `client_id` and `mqtt_topic` inputs
- **Runtime**: one MQTT subscription per entry; a review tracker keeps per-review state (severity, objects, sub-labels, zones, last sent) in memory, replacing the blueprint's `wait_for_trigger` loop. Everything is in code, so improvements reach existing users on upgrade
...
- The **Notification** step of the subentry flow shows an `ActionSelector`, pre-filled with a `supernotify.notify` call using the default title, message, media, priority and actions
- The user sees exactly the `supernotify.notify` UI they'd get in an automation, and can change any field, add `delivery` overrides, `apply_scenarios`, or even add further actions before or after
- The component runs the stored actions with `homeassistant.helpers.script.Script`, passing the review as variables, so templates like `{{ label }}` work as they do in the blueprint
...
- **Apple-compatible video links** (default on) - Apple devices get the HLS `master.m3u8` link and no `clip.mp4` attachment or `clickAction`, as in the [recipe](https://supernotify.rhizomatics.org.uk/latest/recipes/fix_frigate_apple_push/index.md). Off sends the same links to every device, as the blueprint does

## Changes needed in Supernotify

See [Package Support](https://supernotify.rhizomatics.org.uk/latest/developer/rfcs/package_support/index.md), which covers these for all packages. For Frigate, the essentials are snooze enforcement, notification source, lifecycle and tag, cooldown, and the mobile push media, iOS video, tap URL and live view changes.

## Open Questions

- Spike: confirm `ActionSelector` defaults render pre-filled in a subentry flow, and that templates in `data` survive storage and run through `Script` with the review variables
- Spike: confirm `ConditionSelector` renders in config flows, since no core integration uses it there yet
- How does the component know Frigate's camera list, zones and labels during the flow - from the Frigate integration's coordinator data, or the Frigate API?
- Should a default profile be created automatically when a Frigate entry is found, or only offered?
- Naming - see [Packages](https://supernotify.rhizomatics.org.uk/latest/developer/rfcs/packages/#questions)
