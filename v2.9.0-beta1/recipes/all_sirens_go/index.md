...
Use a `chime` delivery with a set of aliases that cope with the variety of implementations, and call it in a notification action, either on its own, or as part of a broader notification with email, mobile push etc
...
This config assumes that the REST commands, switches, sirens and scripts are set up elsewhere in Home Assistant.
...
Use a scenario to automatically apply this delivery to any notification sent via Supernotify with `priority` = `critical`
