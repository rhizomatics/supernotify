...
async def test_apple_drop_mp4(unmocked_config: Context, drop: bool) -> None:
    """With apple_drop_mp4, .mp4 video - the clip URL and an .mp4 attachment - is left out of
    pushes to Apple devices only, which can't always show it; other media is kept"""
    from custom_components.supernotify.transports.mobile_push import OPTION_APPLE_DROP_MP4

    context = unmocked_config
    uut = MobilePushTransport(context)
    devices = {
        "mobile_app_iphone": TrackedDeviceDetails("id001", manufacturer="Apple", model="iPhone"),
        "mobile_app_pixel": TrackedDeviceDetails("id002", manufacturer="Google", model="Pixel"),
    }
    context.hass_api.mobile_app_by_id.side_effect = devices.get  # type: ignore[attr-defined]  # ty: ignore[unresolved-attribute]
    context.configure_for_tests([uut])
    await context.initialize()
    envelope: Envelope = Envelope(
        Delivery("push", {CONF_TRANSPORT: TRANSPORT_MOBILE_PUSH, "options": {OPTION_APPLE_DROP_MP4: drop}}, uut),
        Notification(
            context, message="Person at the gate", action_data={"media": {"clip_url": "https://nvr/api/clip.mp4?h=1"}}
        ),
        target=Target({"mobile_app_id": list(devices)}),
        data={"attachment": {"url": "https://nvr/api/events/1/clip.mp4"}, "image": "https://nvr/api/snapshot.jpg"},
    )

    await uut.deliver(envelope)

    sent = {c.action: c.action_data["data"] for c in envelope.calls if c.action_data}  # type: ignore[index]
    iphone, pixel = sent["mobile_app_iphone"], sent["mobile_app_pixel"]
    assert ("video" in iphone) is not drop
    assert ("attachment" in iphone) is not drop
    assert iphone["image"] == "https://nvr/api/snapshot.jpg"
    assert pixel["video"] == "https://nvr/api/clip.mp4?h=1"
    assert pixel["attachment"] == {"url": "https://nvr/api/events/1/clip.mp4"}


def test_drop_mp4_keeps_other_attachments() -> None:
    from custom_components.supernotify.transports.mobile_push import drop_mp4

    data = {"video": "https://nvr/clip.webm", "attachment": {"url": "https://nvr/snapshot.jpg"}}
    drop_mp4(data)

    assert data == {"video": "https://nvr/clip.webm", "attachment": {"url": "https://nvr/snapshot.jpg"}}
