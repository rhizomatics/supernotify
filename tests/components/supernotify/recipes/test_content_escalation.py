...
""",
    ...
    assert uut.deliveries['plain_email']['delivered_envelopes'][0].priority == 'medium'

    uut = Notification(ctx, "HIGH RISK!! testing 123")
    await uut.initialize()
    await uut.deliver()
    assert list(uut.enabled_scenarios.keys()) == ['high_risk']
    assert uut.deliveries['plain_email']['delivered_envelopes'][0].priority == 'high'
    assert uut.deliveries['plain_email']['delivered_envelopes'][0].message == 'testing 123'
    ...
    assert uut.deliveries['plain_email']['delivered_envelopes'][0].priority == 'low'
    assert uut.deliveries['plain_email']['delivered_envelopes'][0].message == 'testing 123'
