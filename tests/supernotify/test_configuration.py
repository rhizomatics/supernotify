...
async def test_target_definition_merge():
    context = TestingContext(
        deliveries={
...
async def test_target_definition_fixed():
    context = TestingContext(
        deliveries={
            "merge_default_delivery": {
                "transport": "dummy",
...
async def test_target_definition_default():
    context = TestingContext(
        deliveries={
            "merge_default_delivery": {
                "transport": "dummy",
    ...
    await uut.initialize()
    await uut.deliver()
    assert uut.delivered_envelopes[0].target.entity_ids == ["switch.gong"]
...
async def test_target_definition_merge_default():
    context = TestingContext(
        deliveries={
            "merge_default_delivery": {
                "transport": "dummy",
