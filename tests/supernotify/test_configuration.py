...
async def test_default_recipients() -> None:
    context = TestingContext(
        recipients=[{CONF_PERSON: "person.new_home_owner", CONF_TARGET: "dummy.1"},
    ...
    await uut.deliver()
    dummy: DummyTransport = cast(
        "DummyTransport", context.delivery_registry.transports["dummy"])
...
async def test_default_recipients_with_override() -> None:
    ...
    uut = Notification(context, "testing", action_data={
                       ATTR_RECIPIENTS: ["person.new_home_owner"]})
    await uut.initialize()
    await uut.deliver()
    dummy: DummyTransport = cast(
        "DummyTransport", context.delivery_registry.transports["dummy"])
...
async def test_delivery_override_transport() -> None:
    context = TestingContext(
        ...
        transport_configs={
            "dummy": {CONF_DELIVERY_DEFAULTS: {"target": ["media_player.hall"]}}},
        transport_types=[DummyTransport],
    )
    await context.test_initialize()
    ...
    uut = Notification(context, "testing target specified in delivery config", action_data={
                       "delivery": ["quiet_alert"]})
    await uut.initialize()
    ...
    uut = Notification(context, "testing defaulting to transport defaults", action_data={
                       "delivery": ["day_alert"]})
    await uut.initialize()
...
def test_autoresolve_mobile_devices_for_devices(
    ...
    uut = PeopleRegistry([], hass_api)
    uut.initialize()
    device = register_mobile_app(
        uut, person="person.test_user", device_name="phone_bob", title="Bobs Phone")
