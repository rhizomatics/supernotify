...
async def test_exposed_scenario_events(hass: HomeAssistant) -> None:
    ...
    await hass.async_block_till_done()
    assert response == {"simple": [
        "testing", "DEFAULT_notify_entity"], "somebody": ["chime_person"]}
...
async def test_exposed_delivery_events(hass: HomeAssistant) -> None:
    ...
    await hass.async_block_till_done()
    assert response == {"simple": [
        "DEFAULT_notify_entity"], "somebody": ["chime_person"]}
...
async def test_people_configured(hass: HomeAssistant) -> None:
    ...
    assert response["people"][0] == {
        ...
        "mobile_devices": [],
        "delivery": {"chime": {"target": {"entity_id": ["switch.office_bell"]},
                               "data": {"volume": "whisper"},
                               "enabled": True}
                     },
        "target": {
