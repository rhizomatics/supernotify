...
In the action editor, `supernotify.notify` has a **Delivery** dropdown of the configured deliveries. When added, it starts with the deliveries used by default, the same as `supernotify.enquire_implicit_deliveries` lists, so you can add to or take away from what would normally be used.
