...
Define both deliveries and use the `enabled` status on transport to select which one active ( or enable/disable dynamically by changing the entity in Home Assistant between `on` and `off`).
