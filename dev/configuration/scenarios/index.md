...
Each scenario is exposed as `binary_sensor.supernotify_scenario_<name>`, reporting whether its conditions currently hold: `on` or `off`. Only a scenario that has conditions to evaluate gets one, so there is none for a scenario with no conditions, or with `expose_state: false`.
