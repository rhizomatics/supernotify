...
Send the notification with `debug: true` in its `data`, so a `debug_trace` is recorded and kept in the archived copy,
whatever outcomes `diagnostics` selects in the archive configuration, and ensure that archiving to one or more of
file / event / MQTT is enabled in the UI configuration. To keep the trace for every notification of some outcome,
without changing the calls, select that outcome in `diagnostics` instead.
