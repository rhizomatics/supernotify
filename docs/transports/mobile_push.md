...
Apple's mobile apps, on iPhone, iPad and Mac, can't always show an `.mp4` video attached to a push,
such as the clip the Frigate blueprint adds. Set the `apple_drop_mp4` option to leave `.mp4` video
out of pushes to Apple devices, both the `video` URL and an `attachment` pointing at an `.mp4`. Images
are kept, and Android devices still get the video.
...
To switch it on for every mobile push delivery, turn on **Leave video out of Apple pushes** in
**Settings** > **Devices & services** > **Supernotify** > **Configure** > **Delivery Control**.
