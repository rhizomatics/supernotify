...
class DeliveryRegistry:
    ...
    def autogenerate_deliveries(self) -> None:
        ...
        for transport in self.transports.values():
            if any(dc for dc in self._deliveries.values() if dc.get(CONF_TRANSPORT) == transport.name):
                continue
            ...
            if transport.auto_configure and transport.enabled and transport.validate_action(transport_definition.action):
