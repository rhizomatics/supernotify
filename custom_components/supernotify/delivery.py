...
class Delivery(DeliveryConfig):
    ...
    def upgrade_deprecations(self, conf: ConfigType) -> None:
        ...
        # v2.x - smtp transport folded into email
        if conf.get(CONF_TRANSPORT) == TRANSPORT_SMTP:
            _LOGGER.warning(
...
class DeliveryRegistry:
    ...
    async def initialize_transport_deliveries(self, context: Context, transport: Transport) -> None:
        ...
        deliveries_for_this_transport = {
            d: dc
            for d, dc in self._config_deliveries.items()
            if dc.get(CONF_TRANSPORT) == transport.name
            or (transport.name == TRANSPORT_EMAIL and dc.get(CONF_TRANSPORT) == TRANSPORT_SMTP)
        }
    ...
    async def autogenerate_deliveries(self, context: Context) -> None:
        ...
        for transport in [t for t in self.transports.values() if t.enabled]:
            if any(
                dc.get(CONF_TRANSPORT) == transport.name
                or (transport.name == TRANSPORT_EMAIL and dc.get(CONF_TRANSPORT) == TRANSPORT_SMTP)
                for dc in self._config_deliveries.values()
            ):
