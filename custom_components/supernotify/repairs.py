...
DELIVERY_TRANSPORT_BINARY_SENSOR_DEPRECATED_ISSUE_ID = "delivery_transport_binary_sensor_deprecated"
...
def async_create_delivery_transport_binary_sensor_deprecated_issue(hass: HomeAssistant) -> None:
    """Raise a single, persistent, non-fixable warning that delivery and transport
    binary_sensors are going.

    Enabling and disabling a delivery or transport moved to a switch entity, leaving the
    binary_sensor only mirroring it, and there just for backward compatibility. Never deleted by
    us, so once the user has dismissed it, Home Assistant keeps it dismissed rather than raising
    it again.
    """
    ir.async_create_issue(
        hass,
        DOMAIN,
        DELIVERY_TRANSPORT_BINARY_SENSOR_DEPRECATED_ISSUE_ID,
        is_fixable=False,
        is_persistent=True,
        severity=ir.IssueSeverity.WARNING,
        translation_key=DELIVERY_TRANSPORT_BINARY_SENSOR_DEPRECATED_ISSUE_ID,
        learn_more_url="https://supernotify.rhizomatics.org.uk",
    )
